...
namespace LojaVirtual.Libraries.Component
{
    public class ProdutoListagemViewComponent:ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            {
                if (HttpContext.Request.Query["pagina"] != "")
                {
            ...
            if (ViewContext.RouteData.Values.ContainsKey("slug"))
            {
                ...
                Categoria CategoriaPrincipal = _categoriaRepository.FindCategoryById(slug);
                categorias = _categoriaRepository.ObterCategoriaRecursiva(CategoriaPrincipal);
            }
            var ViewModel = new ProdutoListagemViewModel() { lista = _produtoRepository.FindAllProduto(pagina, pesquisa, ordenacao,categorias) };
